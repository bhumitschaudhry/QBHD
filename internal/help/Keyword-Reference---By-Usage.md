...
* [CLS](CLS) (statement) clears the screen and can set the background color in QBHD.
...
* [$CONSOLE]($CONSOLE) (QBHD [Metacommand](Metacommand)) creates a console window throughout the program.
...
* [$SCREENHIDE]($SCREENHIDE) (QBHD [Metacommand](Metacommand)) hides the program window throughout the program until [$SCREENSHOW]($SCREENSHOW) is used.
* [_SCREENHIDE](_SCREENHIDE) hides the main program window in a section of code until [_SCREENSHOW](_SCREENSHOW) is used.
* [$SCREENSHOW]($SCREENSHOW) (QBHD [Metacommand](Metacommand)) displays the main program window throughout the program after [$SCREENHIDE]($SCREENHIDE) has been used.
...
* [_DEFINE](_DEFINE) (statement)  defines a range of untyped variable names according to their first character as a datatype.
* [_FLOAT](_FLOAT) {## numerical [TYPE](TYPE)) values offer the maximum floating-point decimal precision available using QBHD.
...
* [_UNSIGNED](_UNSIGNED) {~ numerical [TYPE](TYPE)) defines an integer numerical value as being positive only in QBHD.
...
The following table describes the error codes that are reported by the **QBHD** compiler, as well as possible causes and solutions:

**TODO**

**N/A means Not Available or Not Applicable to QBHD.**
...
* [ON TIMER(n)](ON-TIMER(n)) (event statement) executes when a timed event occurs. QBHD can use multiple numbered timers.
...
* [TIMER](TIMER) (function) returns the number of seconds past the previous midnite down to a QBHD accuracy of one millisecond.
* [TIMER (statement)](TIMER (statement)) enables, turns off or stops timer event trapping. In QBHD TIMER(n) FREE can free multiple timers. 
...
* [_DISPLAY](_DISPLAY) (statement) renders surfaces visible in the [_DISPLAYORDER](_DISPLAYORDER) previously set in the QBHD program.
...
* [CLS](CLS) (statement) clears a screen page or the program [SCREEN](SCREEN). QBHD can clear with a color.
* [COLOR](COLOR) (statement) sets the current text color attribute or [_RGB](_RGB) value to be used or background colors in some screen modes.
...
* [DECLARE LIBRARY](DECLARE-LIBRARY) required at the END of the block of Library declarations in QBHD.

**QBHD also supports [$INCLUDE]($INCLUDE) text code file Libraries. QBHD does not support QLB Libraries or OBJ files.**
...
**Functions and statements using QBHD's emulated 16 bit memory**
...
**QBHD [Metacommand](Metacommand)s do NOT allow commenting or [REM](REM)!**
 
* [$CHECKING]($CHECKING):OFF/ON (QBHD only) turns event and error checking ON and OFF. ON (default) can only be used after it is OFF.
...
**QBasic [Metacommand](Metacommand)s do not require commenting or [REM](REM) in QBHD!**
...
* [_UNSIGNED](_UNSIGNED) {numerical [TYPE](TYPE)) defines a numerical value as being positive only using QBHD.
...
**QBHD will use the default system printer selected. [_PRINTIMAGE](_PRINTIMAGE) images will be stretched to the paper size setting.**
...
* [ASC (statement)](ASC-(statement)) allows a QBHD program to change a character at any position of a predefined STRING.
...
* DECLARE (BASIC statement) used to tell that a SUB or FUNCTION is created to be used in the program. NOT USED by QBHD!
...
**All Statements and Functions Compile in QBHD Only!**
...
* [ON KEY(n)](ON-KEY(n)) (event statement) executes when a keypress or keypress combination occurs.
* [ON TIMER(n)](ON-TIMER(n)) (event statement) executes when a timed event occurs. QBHD can use multiple numbered timer events.
...
**All Statements and Functions except [SCREEN](SCREEN) Compile in QBHD Only!**
...
* [_SCREENPRINT](_SCREENPRINT) (statement) simulates typing text into a Windows program using the keyboard.
* [$SCREENSHOW]($SCREENSHOW) (QBHD [Metacommand](Metacommand)) displays the main program window throughout the program after [$SCREENHIDE]($SCREENHIDE).
...
* [Dollar_Sign](Dollar_Sign) prefix denotes a QBasic [Metacommand](Metacommand). Only **QBHD**'s event [$CHECKING]($CHECKING) should NOT be commented. 
...
* [Underscore](Underscore) at the end of a code line is used to continue a line of code to the next program line in **QBHD only**.
