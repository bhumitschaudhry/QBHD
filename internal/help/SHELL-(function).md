...
* A SHELL to a QBHD EXE program with an exit return code parameter after [END](END) or [SYSTEM](SYSTEM) will return that code value.
...
Shelling to another QBHD program will return the exit code when one is set in the  program that is run.

```text

'DesktopSize.BAS ** Compile in Windows with QBHD first
...
>  After compiling *DesktopSize.EXE* run the following code in the QBHD IDE. After 1st program is done 3 will appear on screen:

```vb

returncode% = SHELL("DesktopSize") 'replace call with name of any QBHD program EXE
