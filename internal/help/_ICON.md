...
* No image handle designates use of the default QBHD icon or the embedded icon set by [$EXEICON]($EXEICON).

## Description

* If no image handle is passed, the default QBHD icon will be used (all versions). If the [$EXEICON]($EXEICON) metacommand is used, [_ICON](_ICON) without an image handle uses the embedded icon from the binary (Windows only).
...
Function that converts an icon into a temporary bitmap for use in QBHD. Function returns the available image count.
