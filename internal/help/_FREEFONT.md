...
* You cannot free a font which is in use. Change the font to a QBHD default font size before freeing the handle (see example below).
* Predefined **QBHD** font handle numbers can be used before freeing a font:
...
* You cannot free inbuilt/default QBHD fonts nor do they ever need freed. 
...
DO UNTIL EOF(1): found = found + 1
  ...
  IF f& > 0 THEN                  'check for valid handle values > 0
    ...
    PRINT "Press any key."   
    K$ = INPUT$(1)
    _FONT 16                      'use QBHD default font to free tested font
...
_FONT 16
PRINT
PRINT "This is the QBHD default _FONT 16! To change, press any key!"
...
ClearFont: 
IF font& > 0 THEN
    _FONT 16   'change used font to the QBHD 8x16 default font
    _FREEFONT font&
