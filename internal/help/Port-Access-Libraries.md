[INP](INP) and [OUT](OUT) are often used to access port registers, but in QBHD this capability is limited so here are some DLL Libraries:
...
Note: QBHD requires all DLL files to either be with the program or in the `C:\WINDOWS\SYSTEM32` or WOW folder!

At one point there was a sample made available via *dropbox* demonstrating the InpOut32 (x64) libraries with QBHD - but that link has disappeared/expired and the sample is now lost to history.
