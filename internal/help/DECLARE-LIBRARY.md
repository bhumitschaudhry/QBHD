The **DECLARE LIBRARY** declaration allows the use of external library [SUB](SUB) and [FUNCTION](FUNCTION) procedures supported by QBHD.
...
* The Library_filename is needed if a Library is not already loaded by QBHD. Do not include the *.DLL*, *LIB* or *.H* file extension.
* It's always a good idea to try declaring Windows API libraries without a Library_filename first, as most Windows headers are already included in QBHD source.
...
* The declaration can be used with C++ sub-procedures, Windows API and QBHD SDL (versions prior to 1.000)/OpenGL (version 1.000 and up) Libraries.
...
* **NOTE: It is up to the user to document and determine the suitability of all Libraries and procedures they choose to use! QBHD cannot guarantee that any procedure will work and cannot guarantee any troubleshooting help.**
...
> *Explanation:* The SDL Library is included and loaded with QBHD versions prior to 1.000, so these procedures are directly available for use. 

**Using [ALIAS](ALIAS) to create a program SUB or FUNCTION** using **QBHD SDL ONLY**
...
Don't know if a C function is defined by C++ or QBHD? Try using empty quotes.
...
> *Explanation:* The C function 'addone' exists in a library QBHD already links to, but it hasn't been defined as a C function or a QBHD function. By using "" we are telling QBHD the function exists in a library which is already linked to and that it must define the C function before calling it, as well as allowing QBHD code to call it. Trying the above code without the "" will fail.

> **Note: Which libraries are or aren't automatically used in the linking process is not formally defined, nor is it guaranteed to stay that way in future versions of QBHD.**

**QBHD version 1.000 and up produce standalone executables. External DLL files must be distributed with your program.**
