...
* In **QBHD** the [_WIDTH (function)](_WIDTH-(function)) and [_HEIGHT](_HEIGHT) functions will return the text block dimensions in SCREEN 0 only.
...
> Shows an example of each legacy screen mode available to QBasic and QBHD.
...
Making ANY **QBHD legacy screen mode** larger using a SUB that easily converts PRINT to [_PRINTSTRING](_PRINTSTRING). 
