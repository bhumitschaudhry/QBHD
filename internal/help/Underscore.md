...
* The underscore can be anywhere after the code on that line to continue the code to the next line in QBHD.
...
* Modern QBHD keywords are preceded by an underscore, unless [$NOPREFIX]($NOPREFIX) is used. Variables and user procedures names cannot start with a single underscore.
