...
2. [GET (graphics statement)](GET-(graphics-statement)) and [PUT (graphics statement)](PUT-(graphics-statement)) cannot use offscreen coordinates in QB. **QBHD** allows offscreen GETs, but PUT statements can only be used offscreen using the PUT [_CLIP](_CLIP) syntax. Graphic [SCREEN](SCREEN) modes are 1, 2, 7 through 13 only!
