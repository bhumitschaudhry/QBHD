...
So, you would look at the function prototype in the MSDN article, and match the data types of all of the parameters and the return value to QBHD data types. _OFFSET is the type to use for any kind of pointer. MSDN has a list of data types:
...
> Obviously, the QBHD type that corresponds to this is [_UNSIGNED](_UNSIGNED) [LONG](LONG).
...
> This tells us that it is a pointer to a string. Since it is a pointer, the QBHD type it corresponds to is [_OFFSET](_OFFSET).
...
I hope that this answers the question, and adequately demonstrates how to use the Windows API from QBHD. If not, inquire further, and I or someone else will try to answer it. Please point out any mistakes.
