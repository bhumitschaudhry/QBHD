...
**Note: The [PUT (graphics statement)](PUT-(graphics-statement)) options PSET, PRESET, AND, OR and XOR are not available with _PUTIMAGE. QBHD can use [_ALPHA](_ALPHA) of colors to achieve the same results.**
...
source_handle = _LOADIMAGE(**"QBHD.PNG"**, 32)
dx1 = 0: dy1 = 0
...
Using _PUTIMAGE to scroll a larger image created on a separate [_NEWIMAGE](_NEWIMAGE) screen page with QBHD.
