...
* *Speed* (baud rate): 50, 150, 300, 600, 1200, 1800, 2400, **9600** (QBasic's maximum), 19200 or **115200** (**QBHD**'s maximum).
...
* **QBHD** can open any COM*n* port number from 1 to 9.
...
* **Currently, QBHD only supports [OPEN](OPEN) FOR [RANDOM](RANDOM) access using the [GET](GET)/[PUT](PUT) commands in [BINARY](BINARY) mode.**
...
> *Explanation:* QBHD may create error 68 if COM is not found. Use a zero CD, CS, DS or OP timeout value to avoid COM timeouts.

Opening a COM port with the BIN, CS0 and DS0 options in **QBHD**.
