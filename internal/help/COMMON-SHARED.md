...
* In **QBHD** COMMON values are sent using files instead of memory presently.
...
* The COMMON block name designates a certain block of variables that certain program modules may use, the name must be within two forward slashes/ (Ex: /thename/ ). **[Keywords currently not supported by QBHD](Keywords-currently-not-supported-by-QBHD)**
