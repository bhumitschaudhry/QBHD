...
* **NOTE: [LINE INPUT (file statement)](LINE-INPUT-(file-statement)) will work faster in [BINARY](BINARY) than INPUT mode in QBHD to stay compatible with QBasic.**

## Example(s)

Avoiding an INPUT mode or [INPUT (file statement)](INPUT-(file-statement)) read error using a FileExist function. QBHD can use the [_FILEEXISTS](_FILEEXISTS) function.
