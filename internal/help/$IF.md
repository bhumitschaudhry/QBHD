...
* The precompiler comes with some preset values which can be used to help determine which code blocks to include/exclude.  These are:
  ...
  * **VERSION**, which is set to the version of the QBHD compiler. This is a number and can be ordered, see example below.
...
$IF VERSION < 1.5 THEN
    $ERROR Requires QBHD version 1.5 or greater
$END IF

```

*Explanation:* VERSION is a predefined variable that holds the QBHD compiler version. If we know our program needs features only available above a certain version, we can check for that and give the user a helpful error message instead of a confusing error elsewhere in the program.
