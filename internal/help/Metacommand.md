...
> [$CHECKING]($CHECKING){OFF|ON} 'disables QBHD C++ event and error checking (no spaces)

> [$CONSOLE]($CONSOLE) 'creates a QBHD console window throughout the program

> [$RESIZE]($RESIZE):{ON|OFF} 'determines if re-sizing of the program screen by the user is allowed(no spaces)

> [$SCREENHIDE]($SCREENHIDE) 'hides the QBHD program window throughout the program

> [$SCREENSHOW]($SCREENSHOW) 'displays the main QBHD program window
...
* [$SCREENHIDE]($SCREENHIDE) hides the QBHD program window throughout the program until [$SCREENSHOW]($SCREENSHOW) is used.
* [$IF]($IF)...[$END IF]($END-IF) allows selective inclusion of code in the final program.
* **Do not comment out with ' or [REM](REM) QBHD-specific metacommands.**
