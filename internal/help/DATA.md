...
* DATA is best placed after the main program code.
  * **QBHD** DATA can be placed inside a [SUB](SUB) or  [FUNCTION](FUNCTION) procedures.
...
* QBasic comma separations were flexible to allow column alignments when creating them. QBHD removes spacing between commas.
...
How to [RESTORE](RESTORE) and [READ](READ) DATA in a [SUB](SUB) procedure in QBHD. Line labels can be used for multiple DATA fields.
