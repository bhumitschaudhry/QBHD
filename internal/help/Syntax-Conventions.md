...
Items in lowercase are placeholders for information you must supply in the statement (such as a filename$). The QBHD syntax uses data-type suffixes for placeholders that must be a specific data type. Placeholders that can be more than one data type do not have data-type suffixes.
